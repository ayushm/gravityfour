...
function createGameLink() {
	...
	if(nameInput.value) {
		...
		document.getElementById('player').innerHTML = "Your turn!<br><img src='https://cdn1.iconfinder.com/data/icons/function_icon_set/circle_red.png'/>";
	...
	xmlhttp.onreadystatechange = function(){
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
			...
			if(obj.player2.length>0) {
				...
				document.getElementById('waiting-container').innerHTML = "<br><br><h1><span style='color: #96692A;'>"+ opponent + "</span> accepted your invite<br><br>Loading game...</h1>";
				setTimeout(function() {
...
table.onclick = function (e) {
    ...
    //if(x<7 && grid[x+1][y]<1) { //make sure is placed at bottom of column
    	...
    	if(grid[0][y]===0) {
		    ...
		    var queryString = "&gameID=" + gameID +"&turn=2&column="+y+"&rotation="+rand;       // A query string that will be sent to the php script, which will then send the values to the db
	...
	xmlhttp.onreadystatechange = function(){
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
			...
			if(obj.turn==="1") {
			    ...
			    currentPlayer = (currentPlayer%2)+1;
			    var marker = "<img src='https://cdn1.iconfinder.com/data/icons/function_icon_set/circle_red.png'/>";			    
